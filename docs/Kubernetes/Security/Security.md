...
- 모든 유저의 접근은 API Server에 의해 관리된다.
  ...
  - [API 서버의 인증 절차 참고](#11-api-서버의-인증-절차)
...
- Mountable secrets
  ...
  - 서비스어카운트가
    `kubernetes.io/enforce-mountable-secrets="true"` 애노테이션을 가지고 있으면 이 서비스어카운트와 연계된 파드는 Mountable secrets 목록에 있는 시크릿만 마운트할 수 있다.
- Image pull secrets
