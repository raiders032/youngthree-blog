...
- object 키워드는 객체를 선언하거나 동반 객체를 선언할 때 사용합니다.
- **객체 선언**
	- 싱글턴을 정의하는 방법 중 하나입니다.
- **동반 객체**
	- 인스턴스 메서드는 아니지만 어떤 클래스와 관련있는 메서드와 팩토리 메서드를 담을 때 쓰입니다.
	- 동반 객체 메서드에 접근할 때는 동반 객체가 포함된 클래스의 이름을 사용합니다.
- 두 가지 모두 클래스를 정의하면서 동시에 인스턴스를 생성한다는 공통점이 있습니다.

### 8.1 객체 선언: 싱글턴 쉽게 만들기

- 코틀린은 `객체 선언` 기능을 통해 싱글턴을 언어에서 기본 지원합니다.
- 객체 선언(`object`)은 클래스를 정의하고 그 클래스의 인스턴스를 만들어서 변수에 저장하는 모든 작업을 단 한 문장으로 처리합니다.
- 객체 선언에 생성자를 사용할 수 없습니다.
	- 일반 클래스 인스턴스와 달리 싱글턴 객체는 객체 선언문이 있는 위치에서 생성자 호출 없이 즉시 만들어집니다.
- 객체 선언도 클래스와 인터페이스를 상속할 수 있습니다.
- 특정 인터페이스를 구현해야 하는데 상태가 필요하지 않은 경우 object 키워드가 유용합니다.
...
- 코틀린은 자바 static 키워드를 지원하지 않습니다. 
  - 패키지 수준의 최상위 함수: 자바의 정적 메서드 역할을 대신
  - 객체 선언: 자바의 정적 메서드 중 코틀린 최상위 함수가 대신할 수 없는 경우 사용
  - 대부분의 경우 최상위 함수를 사용하는 것이 권장됩니다.
  - 최상위 함수는 private으로 표시된 클래스 비공개 멤버에 접근할 수 없습니다.
  - private 멤버에 접근이 필요한 대표적인 예로 팩토리 메서드가 있습니다. 팩토리 메서드는 객체 생성을 위해 클래스의 private 멤버에 접근해야 합니다.
  - 이러한 경우 동반 객체를 사용합니다.
- 클래스 안에 정의된 `object` 중 하나에 `companion`이라는 키워드를 붙이면 그 클래스의 동반 객체로 만들 수 있습니다.
	- 동반 객체는 클래스 안에 정의된 일반 객체입니다.
- 동반 객체의 프로퍼티나 메서드에 접근하려면 동반 객체가 정의된 클래스 이름을 사용합니다.
- 동반 객체는 자신을 둘러싼 클래스의 모든 private 멤버에 접근할 수 있습니다.
	- 따라서 동반 객체는 팩토리 메서드를 구현하기 가장 적합한 위치입니다.
...
- 객체 멤버에 접근할 때 자신을 감싸는 클래스의 이름을 통해 직접 접근할 수 있습니다.
...
- 생성자의 접근 지시자가 private이므로 팩터리 메서드를 통해서만 인스턴스를 만들 수 있습니다.
- 클래스 이름을 사용해 동반 객체의 메서드를 호출할 수 있습니다.
