...
- 모든 중단 함수는 또 다른 중단 함수에 의해 호출되어야 하기 때문에 시작되는 지점이 반드시 있습니다.
	- 코루틴 빌더가 그 역할을 합니다.
	- 코루틴 빌더는 일반 함수와 중단 가능한 세계를 연결시키는 다리가 됩니다.
...
- `Deferred<T>`는 `Job`의 하위 타입입니다. 즉, `Deferred`는 `Job`의 모든 기능을 상속받으며, 추가로 결과 값을 반환하는 기능을 가지고 있습니다.
...
---
title: "코루틴 실행 옵션 (CoroutineStart)"
description: "코틀린 코루틴의 CoroutineStart 옵션에 대한 상세 가이드입니다. DEFAULT, LAZY, ATOMIC, UNDISPATCHED 옵션의 특징과 실용적인 사용 사례를 코드 예제와
함께 설명합니다."
tags: ["COROUTINE", "KOTLIN", "CONCURRENCY", "BACKEND", "ANDROID"]
keywords: ["코루틴", "coroutine", "코루틴스타트", "CoroutineStart", "코틀린", "kotlin", "지연실행", "lazy coroutine", "동시성", "concurrency", "비동기", "async", "launch", "코루틴빌더", "coroutine builder"]
draft: false
hide_title: true
---
