...
- 캐시는 데이터를 휘발성 메모리에 두므로 영속적으로 보관할 데이터는 캐시에 두는 것은 바람직하지 않습니다.
- 중요한 데이터는 여전히 persistent data store에 저장해야 합니다.

### 4.2 만료 정책

- 캐시에 보관된 데이터의 만료 정책을 정해야합니다.
- 만료 기한이 없는 경우 데이터가 캐시를 가득 채울 것입니다.
- 만료 기한이 너무 짧은 경우 데이터베이스를 더 많이 접근하게 될 것이다
- 만료 기한이 너무 긴 경우 캐시된 데이터가 원본과 차이가 날 경우가 많아질 것입니다.

### 4.4 Cache Stampede

- 만료 기한이 너무 짧은 경우 캐시에 대한 요청이 동시에 들어오면 캐시가 만료되어 데이터베이스에 대한 요청이 동시에 들어가게 됩니다.
- 혹은 캐시가 전부 같은 시간에 만료되도록 구현하면 같은 문제가 발생할 수 있습니다.
	- 예를 들어, 매일 자정 캐시를 갱신한다고하면 데이터베이스로 대량의 요청이 들어가게 될 것입니다.

**해결 방안**

- 해결방안으로 Jitter를 사용하거나 캐시 갱신을 랜덤한 시간에 수행하도록 하는 방법이 있습니다.
- 지터는 전자 신호를 읽는 과정에서 발생하는 짧은 지연 시간을 의미합니다.
- 캐시 만료 시간에 랜덤한 값을 더해주면 캐시 갱신 시간이 겹치는 것을 방지할 수 있습니다.

### 4.4 일관성(consistency)

- 일관성이란 데이터 저장소의 원본과 캐시 내의 사본이 같은지의 여부를 의미합니다.
- 저장소의 데이터를 갱신하는 연산과 캐시를 갱신하는 연산이 단일 트랜잭션으로 처리되지 않는 경우 일관성이 깨질 수 있습니다.

### 4.5 장애 대처

- 캐시 서버를 한 대만 두는 경우 해당 서버는 단일 장애 지점(Single Point of Failure)이 되어버립니다.
- 어떤 특정 지점에서의 장애가 전체 시스템의 동작을 중단시켜버릴 수 있는 경우 이 특정 지점을 `Single Point of Failure`라고 합니다.
- 결과적으로 `Single Point of Failure`를 피하려면 여러 지역에 걸쳐 캐시 서버를 분산해야 합니다.

### 4.6 메모리 크기 설정

- 캐시 메모리 크기는 얼마나 크게 잡아야 할까?
- 캐시 메모리가 너무 작으면 액세스 패턴에 따라 데이터가 너무 자주 캐시에서 밀려나버려 캐시의 성능이 떨어지게 됩니다.
- 이를 막을 방법으로 캐시 메모리를 과할당 하는 것이 있습니다.
	- 이렇게 하면 캐시에 보관될 데이터가 갑자기 늘어났을 때 생길 문제도 방지할 수 있습니다.

### 4.7 데이터 방출 정책

- 캐시가 꽉 차버리면 추가로 캐시에 데이터를 넣어야 할 경우 기존 데이터를 내보내야 합니다.
- 이것을 캐시 데이터 방출 정책이라고 합니다.
- 가장 널리 쓰이는 방식은 LRU로 마지막으로 사용된 시점이 가장 오래된 데이터를 내보내는 정책이 있습니다.
- 방출 정책에 대해서는 아래에서 자세히 설명합니다.
...
- 캐시 메모리가 가득 차면 새로운 데이터를 저장하기 위해 기존 데이터를 제거해야 합니다.
- 이때 어떤 데이터를 제거할지 결정하는 알고리즘을 캐시 방출 정책(Eviction Policy)이라고 합니다.
- 각 정책은 고유한 특성이 있으며, 애플리케이션의 액세스 패턴에 따라 적합한 정책을 선택해야 합니다.

### 5.1 LRU (Least Recently Used)

- 가장 오랫동안 사용되지 않은 데이터를 먼저 제거하는 정책입니다.
- 가장 널리 사용되는 캐시 방출 정책입니다.
- 시간적 지역성(Temporal Locality)을 활용합니다.
	- 최근에 사용된 데이터는 가까운 미래에도 다시 사용될 가능성이 높다는 원리입니다.
- 장점:
	- 구현이 비교적 간단합니다.
	- 대부분의 워크로드에서 좋은 성능을 보입니다.
- 단점:
	- 일회성 대량 스캔 시 유용한 캐시 데이터가 밀려날 수 있습니다.
	- 접근 빈도를 고려하지 않습니다.

### 5.2 LFU (Least Frequently Used)

- 사용 빈도가 가장 낮은 데이터를 먼저 제거하는 정책입니다.
- 각 항목의 접근 횟수를 카운트하여 관리합니다.
- 장점:
	- 자주 사용되는 데이터를 오래 보관합니다.
	- 일회성 대량 스캔에 강합니다.
- 단점:
	- 과거에 많이 사용되었지만 더 이상 사용되지 않는 데이터가 캐시에 남을 수 있습니다.
	- 빈도 카운터 관리로 인한 오버헤드가 발생합니다.
	- 새로운 데이터가 축적될 기회를 얻기 전에 제거될 수 있습니다.

### 5.3 FIFO (First In First Out)

- 가장 먼저 캐시에 들어온 데이터를 먼저 제거하는 정책입니다.
- 큐(Queue) 자료구조를 사용하여 구현합니다.
- 장점:
	- 구현이 매우 간단합니다.
	- 오버헤드가 적습니다.
- 단점:
	- 데이터의 사용 패턴을 전혀 고려하지 않습니다.
	- 자주 사용되는 데이터도 오래되면 제거될 수 있습니다.

### 5.4 Random Replacement

- 무작위로 데이터를 선택하여 제거하는 정책입니다.
- 장점:
	- 구현이 가장 간단합니다.
	- 오버헤드가 거의 없습니다.
	- 특정 패턴에 의한 최악의 경우를 피할 수 있습니다.
- 단점:
	- 예측 불가능한 성능을 보입니다.
	- 최적의 결과를 보장하지 않습니다.
...
- 가장 최근에 사용된 데이터를 먼저 제거하는 정책입니다.
- LRU와 반대되는 정책입니다.
- 특수한 상황에서 유용합니다:
	- 한 번 접근한 데이터는 당분간 다시 접근하지 않는 패턴
	- 순차적으로 한 번씩만 접근하는 대용량 데이터 스캔

### 5.6 정책 선택 가이드

| 정책 | 적합한 상황 | 부적합한 상황 |
|------|------------|--------------|
| LRU | 일반적인 웹 애플리케이션, 시간적 지역성이 높은 경우 | 대량 스캔이 빈번한 경우 |
| LFU | 인기 콘텐츠 캐싱, 접근 빈도 차이가 큰 경우 | 접근 패턴이 시간에 따라 변하는 경우 |
| FIFO | 단순한 캐시, 시간 기반 만료가 중요한 경우 | 접근 패턴이 중요한 경우 |
| Random | 빠른 구현이 필요한 경우, 워크로드 예측이 어려운 경우 | 예측 가능한 성능이 필요한 경우 |
| MRU | 순차 스캔, 한 번 접근한 데이터를 재접근하지 않는 경우 | 일반적인 웹 애플리케이션 |

## 6 캐시 고급 활용

### 6.1 캐시 계층화

- 캐시 계층화는 캐시를 여러 계층으로 나누어 데이터를 저장하는 방법입니다.
- 예를 들어, 핫 캐시는 매우 자주 조회되는 데이터를 저장하고, 웜 캐시는 중간 빈도로 조회되는 데이터를 저장하고, 콜드 캐시는 드문 빈도로 조회되는 데이터를 저장하는 식입니다.

### 6.2 시스템 재시작 대비

- 캐시 서버를 재시작하면 캐시 데이터가 모두 사라집니다.
- 이 때 미리 캐시에 데이터를 채워 넣는 방식인 캐시 워밍(Cache Warmup)을 통해 초기 성능 저하를 방지할 수 있습니다.

## 7 CDN(Content delivery network)

- CDN은 정적 콘텐츠를 전송하는 데 쓰이는 지리적으로 분산된 네트워크입니다.
- 이미지, 비디오, CSS, JavaScript 파일을 등을 캐시할 수 있습니다.

### 7.1 CDN 동작과정

1. 사용자 A가 이미지 URL을 이용해 이미지에 접근합니다.
	- URL의 도메인은 CDN 서비스 사업자가 제공한 것
2. CDN 서버의 캐시에 해당 이미지가 없는 경우 서버는 원본 서버에 요청하여 이미지를 가져옵니다.
	- 원본 서버는 웹 서버일 수도 있고 아마존 S3 같은 온라인 저장소일 수 있습니다.
3. 원본 서버가 CDN 서버에게 이미지를 반환합니다.
	- 응답 헤더에 TTL 값이 명시되어 있습니다.
	- TTL은 해당 데이터의 만료시간을 의미합니다.
5. CDN 서버는 파일을 캐시하고 사용자 A에게 반환합니다.
	- 이미지는 TTL에 명시된 시간까지 캐시됩니다.
5. 만료되지 않은 이미지에 대한 요청은 캐시를 통해 처리합니다.

### 7.2 CDN 사용 시 고려사항

- CDN은 보통 third party에서 운영하므로 CDN으로 들어가고 나가는 데이터 전송 양에 따라 요금을 내게 됩니다.
  - 따라서 자주 사용되지 않는 콘텐츠는 CDN에 캐싱하지 말아야 합니다.
- 적절한 만료시간을 설정해야 하는데 너무 길면 신선도가 떨어지고 너무 짧으면 원본 서버에 빈번히 접속해 성능상 좋지 않습니다.
- CDN 자체가 죽을 경우 애플리케이션이 어떻게 동작해야 하는지 고려해야 합니다.
	- 가령 CDN이 응답하지 않을 경우 원본 서버로로 부터 직접 콘텐츠를 가져오도록 클라이언트를 구성합니다.

### 7.3 장점

- 정적 콘텐츠를 더이상 웹 서버에서 서비스하지 않으며 CDN을 통해 제공하여 더 나은 성능을 보장합니다.
- 캐시가 데이터베이스 부하를 줄여줍니다.
