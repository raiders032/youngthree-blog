...
- API Gateway는 전 세계 여러 AWS 리전에서 사용할 수 있으며, 리전별로 독립적인 API를 생성하고 관리할 수 있습니다.
- **리전별 API**: 각 리전에서 독립적으로 API를 생성하고 관리할 수 있어, 지역별 규정 준수 요구사항을 충족하고 지연 시간을 최소화할 수 있습니다.
- **엣지 최적화 엔드포인트**: CloudFront의 전 세계 엣지 로케이션을 활용하여 글로벌 사용자에게 더 빠른 응답 시간을 제공합니다.
- **프라이빗 API**: VPC 내에서만 접근 가능한 프라이빗 API를 특정 리전에 생성할 수 있습니다.
- **교차 리전 API 관리**: 여러 리전에 배포된 API를 중앙에서 관리하고 모니터링할 수 있습니다.
