...
1. Task가 EventLoop에서 실행되기 위해 Channel.eventLoop().execute(Task) 메서드를 통해 제출됩니다.
...
- 비동기 트랜스포트의 경우
- 적은 수의 EventLoop 사용
- 여러 Channel이 EventLoop 공유
- Channel 생성 시 EventLoopGroup이 EventLoop 할당
- 라운드 로빈 방식으로 균형있게 분배
- Channel은 수명 주기 동안 동일한 EventLoop 사용

### 4.3 ThreadLocal 사용 시 주의사항

- EventLoop가 여러 Channel을 처리하므로:
- ThreadLocal 값은 모든 관련 Channel에서 공유
- 상태 추적용으로는 부적합
- 무상태 컨텍스트에서 무거운 객체 공유 용도로 적합
