...
- epoll_create1
	- epoll 인스턴스를 생성하고 해당 파일 디스크립터를 반환합니다.
	- flags는 생성 옵션을 지정하며, 0 또는 EPOLL_CLOEXEC를 사용할 수 있습니다.
- epoll_ctl
	- epoll 인스턴스의 관심 목록을 수정합니다.
	- op: EPOLL_CTL_ADD(추가), EPOLL_CTL_MOD(수정), EPOLL_CTL_DEL(삭제)
	- event 구조체로 모니터링할 이벤트 유형을 지정합니다.
- epoll_wait
	- 이벤트 발생을 대기하고, 준비된 파일 디스크립터 목록을 반환합니다.
	- timeout이 -1이면 이벤트가 발생할 때까지 무한 대기합니다.
