...
- 네트워크에 연결된 장치가 라우터이든 일반 서버이든, 모든 기계는 이 특수한 번호를 가지고 있어야 합니다.
- [더 자세히 알아보기](../IP/IP.md)(IP 주소 참고)
:::
...
- 국가 코드 최상위 도메인
	- 여기에는 국가 또는 주와 관련된 모든 도메인이 포함됩니다.
	-  예) .uk, .us, .ru, .jp
- TLD 네임 서버는 Registry(등록소)가 관리합니다.
	- .com, .net TLD 네임 서버는 VeriSign Global Registry Services 라는 기업이 관리합니다.
...
- DNS의 쿼리 방식은 크게 반복적 쿼리와 재귀적 쿼리로 구분할 수 있습니다.

### 5.1 `example.com` 의 IP 주소를 찾는 과정
...
- DNS 데이터베이스는 매핑 정보를 리소스 레코드라는 단위로 저장합니다.
- 다양한 종류의 정보를 저장하는 여러 유형의 리소스 레코드가 있습니다.

### 6.1 A 레코드

- A 레코드는 도메인 주소를 IPv4주소로 변환하는 레코드입니다.
- 클라이언트가 DNS에 질의한 도메인 주소를 A 레코드에 설정된 IP 주소로 응답합니다.

### 6.2 AAAA 레코드

- AAAA 레코드는 도메인 주소를 IPv6 주소로 변환하는 레코드입니다.
- A 레코드와 동일한 기능을 수행하지만 IPv6 주소를 사용합니다.
- IPv6 주소 체계는 128비트로 이루어져 있으며, A 레코드의 IPv4 주소보다 더 많은 주소를 제공합니다.
...
- NS 레코드는 네임 서버(Name Server) 레코드를 정의합니다.
- 특정 도메인에 대한 DNS 쿼리를 처리할 권한이 있는 네임 서버를 지정합니다.
- 도메인의 DNS 정보를 관리하는 네임 서버를 가리키며, 도메인의 권한 있는 네임 서버를 나타냅니다.
- 여러 개의 NS 레코드를 설정하여 도메인의 가용성과 신뢰성을 높일 수 있습니다.

### 6.5 MX 레코드

- MX 레코드는 메일 서버(Mail Exchange) 레코드를 정의합니다.
- 특정 도메인에 대한 메일 서버를 지정합니다.

## 참고 자료

- [생활코딩](https://www.youtube.com/watch?v=zrqivQVj3JM&list=PLuHgQVnccGMCI75J-rC8yZSVGZq3gYsFp&index=1)
- [AWS Route 53 공식 문서](https://aws.amazon.com/ko/route53/what-is-dns/)
- [Cloudflare DNS 서버 유형](https://www.cloudflare.com/ko-kr/learning/dns/dns-server-types/#authoritative-nameserver)