...
- 이번 장에서 배울 내용
  - 분산 환경의 메시지 전달 IPC가 어떤 것인지 알아본다.
  - 클라이언트-서버 애플리케이션이 무엇인지 알아본다.
  - 입출력 연산에서 다중 스레드/프로세스를 활용할 때 한계점을 이해한다.
  - 논블로킹 연산과 이를 이용해 입출력 중신 연산을 가리는 방법을 배운다.


### 10.1 분산 컴퓨팅의 세계

- 동시성의 범위는 이미 오래전에 단일 컴퓨터의 한계를 넘어섰다.
- 발전된 기술을 수백수천 대의 컴퓨터가 서로 연결된 세상을 가능케 했다.

### 10.2 클라이언트-서버 모델

- 클라이언트-서버 모델은 분산 컴퓨팅의 기본 모델이다.
- 이 모델은 클라이언트와 서버 이렇게 두 프로세스로 구성된다.

#### 네트워크 소켓

- 네트워크 소켓에도 몇 가지 종류가 있지만 여기서는 TCP/IP 소켓을 다룬다.
- TCP/IP 소켓은 데이터 전송을 보장하는 특성이 있어 가장 널리 사용된다.
- 이 과정에서 정보를 주고받기 전에 두 프로세스의 동의가 필요하다.
- 이들 프로세스는 통신 세션이 지속되는 내내 접속을 유지한다.
- 네트워크 소켓은 운영체제에서 사용하는 네트워크 통신의 추상이다.
- 소켓은 IP 주소와 포트 이렇게 두 가지 값을 갖느다.

#### C10k 문제

- 현대적인 서버 애플리케이션은 수백에서 수만에 이르는 클라이연터 요청(스레드)을 거의 지연 없이 동시에 처리할 수 있다.
- 운영체제는 스레드의 실행 준비 여부와 상관없이 모든 스레드에 CPU 시간을 배정한다.
- 예를 들어, 소켓으로 들어올 데이터를 기다리는 스레드는 데이터가 들어오기까지 수천 번 이상 CPU 시간을 배정받아 낭비한다.
- 스케줄러가 가진 CPU 타임이 10ms라고 가정해보자.
  - 두 스레드가 있다마면 한 스레드가 배정받는 시간은 5ms이다.
  - 다섯개라면? 2ms 스레드가 1000개라면?
  - 이런 상황이라면 컨텍스트 스위칭에만 시간을 쓸 뿐 실질적인 일을 할 수 없다.
  - 이를 해결하려면 타임 슬라이스의 크기를 일정 시간 이상으로 제한해야한다.
  - 최저 크기를 2ms로 잡으면 1000개 스레드가 있다면 스케줄러에 필요한 CPU 시간은 2초가 된다.
  - 스레드 10,000개를 동시에 실행하면 자신의 타임 슬라이스를 다 쓴다고 가정할 때 20초가 걸린다. 20초는 너무 길다.
- 이렇게 서버가 10,000개의 동시 접속을 처리하기 어려워지는 확장성의 문제를 C10k 문제라고 한다.
  - 당시보다 기술이 발전하면서 현재는 c10m 문제로 확장됐다.
  - 1000만개의 클라이언트 요청을 동시에 처리할 수 있어야 한다.
- 안타깝지만 스레드로는 c10k 문제를 해결할 수 없다.

## chpater 11: 이벤트 기반 동시성

### 11.6 리액터 패턴

- 이벤트가 발생할 떄까지 기다렸다가 발생한 이벤트를 처리하는 이러한 형태의 이벤트 반복문은 매우 널리 쓰여 디자인 패턴으로까지 자리 잡았다.
- 이것이 바로 리액터 패턴이다.
- 리팩터 패턴이 성립하려면 이벤트 소스, 이벤트 핸들러, 동기 이벤트 디멀티플렉서, 리액터 구조가 있어야 한다.
- 이벤트 소스는 이벤트를 생성하는 요소다.
  - 파일, 소켓, 타이머, 동기화 객체등이 있다.
- 이벤트 핸들러는 어떤 이벤트 소스에서 들어온 요청을 처리하는 콜백함수를 가리킨다.
- 동기 이벤트 디멀티플렉서는 운영체제가 제공하는 select 같은 이벤트 알림 시스템을 어렵게 부르는 말이다.
- 리액터, 다른 말로 이벤트 반복문은 이러한 과정 전체를 관장한다.
  - 이벤트마다 해당 이벤트를 처리할 콜백을 등록하고
  - 그 이벤트가 발생하면 등록된 이벤트 핸들러 또는 콜백에 이벤트 처리를 맡긴다.
